...
document.addEventListener("DOMContentLoaded", function () {
  var calcButton = document.querySelector("#calc form button");
  var licenceCheckbox = document.querySelector("#calc .licence");
  var error = document.querySelector("#calc .display .error");
  var results = document.querySelector("#calc .display .results");
  var LearningPermitExtensions = {
    firstExtensionInMonths: 8,
    secondExtensionInMonths: 4
  };
  var DrivingLicenceExtensions = {
  ...
  function updateTitle() {
    var LicenceType = getLicence();
    var title = document.querySelector("#calc .intro h2");
    var description = document.querySelector("#calc .intro p");
    title.innerHTML = titleCase(LicenceType);

    if (LicenceType === "learner permit") {
      description.innerHTML = "Please enter the expiry date as it appears in 4b of your ".concat(LicenceType, " card.");
    }

    if (LicenceType === "driving licence") {
      description.innerHTML = "Please enter the expiry date as it appears in 4b on your ".concat(LicenceType, " card or in the third column alongside the categories on your paper driving licence.");
    }
  ...
  function handle(e) {
    e.preventDefault();
    var expiryDate = document.querySelector("#calc #expiry");

    if (!expiryDate.value || expiryDate.value === "dd/mm/yyyy") {
      displayError("Please enter an expiry date.");
    } else {
      var renewalDates = getRenewalDates(expiryDate.value, getLicence());
  ...
  function displayResults(result) {
    ...
    if (result.extension) {
      results.innerHTML = "\n      <p>New Expiry Date: ".concat(result.extension.toLocaleString("en-IE", options), "</p>");
    }

    if (result.renewal) {
      results.innerHTML += "\n      <p>Earliest Renewal: ".concat(result.renewal.toLocaleString("en-IE", options), "</p>");
    }
    ...
    if (result.shouldRenew) {
      results.innerHTML += "<p>You need to renew now. <a href='/booking-service.html'>Click here to continue to the booking service page</a></p>";
    } else {
      results.innerHTML += "<p>You do not need to renew. There is no need to make an appointment. It will be refused.</p>";
    }
  }
  /**
  ...
  function titleCase(str) {
    ...
    var wordArr = strLowerCase.split(" ").map(function (currentValue) {
  ...
  function calculateLearnerPermitDates(expiryDate, licence) {
    ...
    if (expiry < March) {
      return {
        message: "Expired more than 3 months ago. Renewal required immediately."
      };
    }
    ...
    if (expiry > June28 && expiry < July1) {
      return {
        ...
        renewal: addMonths(addDays(expiry, 1), licence.firstExtensionInMonths - 3)
      };
    }
    ...
    if (expiry >= July && expiry <= October) {
      return {
        ...
        renewal: addMonths(addDays(expiry, 1), licence.secondExtensionInMonths - 3)
      };
    }
  ...
  function withinRenewalWindow(earliestRenewalDate) {
    ...
    if (today > earliestRenewalDate || typeof earliestRenewalDate === "undefined") {
      return true;
    }
